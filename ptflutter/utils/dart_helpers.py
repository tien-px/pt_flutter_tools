...
def get_path_from_content_root(path: str):
    current_working_dictionary = os.path.abspath(os.getcwd()).replace("\\", "/")
...
def get_dart_class_name(content):
    try:
        regex = r"(?<=^class )([\S\s]*?)(?= implements .*UseCaseType)"
...
def insert_dart_file_by_regex(path, regex, replacement):
    try:
        with open(path, "r+", encoding="utf8") as file:
            ...
            if matches:
                content = insert_str(
                    content, "{}".format(replacement), matches.__next__().span("G1")[1]
                )
            write_content_to_file(file, content)
    except Exception as e:
        logError(str(e))
...
def insert_dart_imports(path, list_import):
    ...
    try:
        with open(path, "r+", encoding="utf8") as file:
            content = file.read()
            current_import_files = re.findall(import_reg_exp, content, re.MULTILINE)
            new_import_files = "\n".join(
